...
#define ESP_PANEL_DRIVERS_LCD_USE_ALL                   (0)
    ...
    // GC9B72 uses the compatible GC9B71 transport/draw backend.
    #define ESP_PANEL_DRIVERS_LCD_USE_GC9B71            (0)
    #define ESP_PANEL_DRIVERS_LCD_GC9B72_COMPAT         (0)
 ...
 * |   1   |   1   |                 Yes                 |              Yes           |
 */
// CST3530由自定义ESP-IDF旧版I2C驱动管理，不编译ESP Panel内置触摸驱动
...
#define ESP_PANEL_DRIVERS_EXPANDER_USE_ALL                      (0)
    ...
    // The board uses a TCA9554/TCA95XX 8-bit I/O expander at 0x20.
    // This must be enabled for Board::begin() and P2 (BL_EN) control to work.
 ...
 * Set to `1` to enable, `0` to disable.
 */
// BL_PWM由GPIO38控制（低电平有效），BL_EN由TCA9554 P2控制（高电平有效），暂不启用LEDC PWM调光
