...
/* UI只读取此缓存，避免与LVGL输入回调重复访问I2C */
static LVGLPortTouchState latestTouchState;

bool lvgl_port_get_touch_state(LVGLPortTouchState *state)
{
    if (state == nullptr) {
        return false;
    }

    *state = latestTouchState;
    return true;
}
...
{
    // 从LVGL的user_data中取出自定义GT911驱动对象
    auto *tp =
        static_cast<GT911Touch *>(indev_drv->user_data);
    ...
    // 驱动对象无效或读取失败时保持释放状态
    if ((tp == nullptr) || !tp->readPoint(point)) {
        return;
    }

    /* 将同一次读取结果提供给触摸测试UI */
    latestTouchState.x = point.x;
    latestTouchState.y = point.y;
    latestTouchState.pressure = point.pressure;
    latestTouchState.points = point.points;
    latestTouchState.pressed = point.pressed;

    // GT911上报有效按下时，将坐标交给LVGL
    if (point.pressed) {
        data->point.x = point.x;
        data->point.y = point.y;
        data->state = LV_INDEV_STATE_PRESSED;
    }
    // 记录上一次触摸状态，避免LVGL高速轮询造成串口刷屏
...
if (point.pressed) {
    ...
    // 按下瞬间立即输出；持续按压时每100毫秒输出一次坐标
    if (!lastPressed || (now - lastPrintTime >= 100)) {
        Serial.printf(
            "TP pressed: x=%u, y=%u, pressure=%u, id=%u\n",
            point.x,
            point.y,
            point.pressure,
            point.id
...
/**
 * @brief 向LVGL注册GT911指针输入设备
 */
static lv_indev_t *indev_init(GT911Touch *tp)
{
    if (tp == nullptr) {
        return nullptr;
    }
    ...
    /*
    * 将LVGL长按识别时间设置为500毫秒。
    * 颜色测试按钮按住达到该时间后才会切换纯色显示。
    */
    indevDriver.long_press_time = 500;

    // 将GT911注册成鼠标/触摸屏类型的指针设备
    indevDriver.type = LV_INDEV_TYPE_POINTER;
    indevDriver.read_cb = touchpad_read;
    indevDriver.user_data = tp;

    return lv_indev_drv_register(&indevDriver);
}
...
bool lvgl_port_init(LCD *lcd, GT911Touch *tp)
{
