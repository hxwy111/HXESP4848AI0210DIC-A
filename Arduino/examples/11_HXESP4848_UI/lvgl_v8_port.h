...
/**
 * @brief 提供给UI界面的最近一次触摸状态
 *
 * UI只能读取这个缓存，不能直接再次访问GT911，
 * 避免与LVGL输入回调争用I2C触摸数据。
 */
struct LVGLPortTouchState {
    uint16_t x = 0;
    uint16_t y = 0;
    uint8_t pressure = 0;
    uint8_t points = 0;
...
/**
 * @brief 将LCD和自定义GT911触摸驱动接入LVGL
 *
 * @param lcd LCD面板对象指针，不能为nullptr
 * @param tp  GT911触摸对象指针，不使用触摸时可以传入nullptr
 *
 * @return true表示初始化成功，false表示初始化失败
 */
bool lvgl_port_init(esp_panel::drivers::LCD *lcd, GT911Touch *tp);

/**
 * @brief 读取LVGL最近一次获得的触摸状态
 *
 * @param state 用于接收坐标、压力及按压状态，不能为nullptr
