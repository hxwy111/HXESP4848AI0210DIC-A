...
static esp_err_t tca9554_write_reg(uint8_t reg, uint8_t data)
{
    ...
    return i2c_master_write_to_device(I2C_PORT, TCA9554_ADDR, buf, sizeof(buf), pdMS_TO_TICKS(1000));
}

/* 读 TCA9554 寄存器：先写寄存器地址，再用重复起始条件读回 1 字节 */
static esp_err_t tca9554_read_reg(uint8_t reg, uint8_t *data)
{
    return i2c_master_write_read_device(I2C_PORT, TCA9554_ADDR, &reg, 1, data, 1, pdMS_TO_TICKS(1000));
}
...
void app_main(void)
{
    ...
    /* 初始化 I2C0（与 QMI8658 六轴 / CST3530 触摸共用一条总线） */
    i2c_config_t conf = {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "TCA9554 (0x%02X) 无响应: %s，请检查 I2C 接线/供电", TCA9554_ADDR, esp_err_to_name(err));
        return;
    }
