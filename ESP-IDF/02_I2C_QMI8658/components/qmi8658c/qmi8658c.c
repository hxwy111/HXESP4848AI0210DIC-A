...
{
  if(I2C_read_buff(g_imu.slave,reg,buf,len) != ESP_OK)
	{
...
unsigned char qmi8658_get_id(void)
{
	...
	/* Match the Arduino reference: probe 0x6B first, then 0x6A fallback. */
...
void qmi8658c_example(void* parmeter)
{
  ...
  float temp;
	...
	/* Allow the sensor supply/reset state to settle after MCU reset. */
	vTaskDelay(pdMS_TO_TICKS(50));
	if (!qmi8658_init())
	{
		printf("QMI8658 not found at 0x6B/0x6A; stopping sensor task.\n");
		vTaskDelete(NULL);
		return;
	}
