...
/* 定义后输出单位为 mg、dps；默认输出单位为 m/s²、rad/s。 */
...
{
  if(I2C_writr_buff(g_imu.slave,reg,value,len) != ESP_OK)
	{
...
{
	if (buf == NULL || len == 0 || len > UINT8_MAX) {
		return 0;
	}
	if(I2C_read_buff(g_imu.slave,reg,buf,(uint8_t)len) != ESP_OK)
	{
...
/* 根据芯片安装方向 layout 对加速度和角速度的三轴进行交换及取反。 */
...
/*
 * 静止状态下累计样本并计算零偏。
 * sensor=1 表示加速度计，sensor=2 表示陀螺仪。
 */
...
{
	// CTRL3 用于配置陀螺仪动态量程和输出数据率
	unsigned char ctl_dada; 

	// 保存灵敏度系数，读取原始数据时据此换算物理量
	switch (range)
	{
...
/*
 * 发送 CTRL9 功能命令，并轮询状态位等待命令完成；
 * 完成后写入 NOP 清除命令状态，避免影响下一条命令。
 */
void qmi8658_send_ctl9cmd(enum qmi8658_Ctrl9Command cmd)
{
	...
	qmi8658_write_reg(Qmi8658Register_Ctrl9, (unsigned char)cmd);	// 将功能命令写入 CTRL9
	...
	while(((status1&cmd_done)!=cmd_done)&&(count++<100))		// 等待 StatusInt bit7 置位
	{
		qmi8658_delay(1);
		qmi8658_read_reg(status_reg, &status1, 1);
	}
	...
	qmi8658_write_reg(Qmi8658Register_Ctrl9, qmi8658_Ctrl9_Cmd_NOP);	// 写入 NOP 清除命令
	count = 0;
	qmi8658_read_reg(status_reg, &status1, 1);
	while(((status1&cmd_done)==cmd_done)&&(count++<100))		// 等待 StatusInt bit7 清零
	{
...
/*
 * 一次性读取三轴加速度和三轴角速度原始值，并换算为配置的物理单位。
 * 默认单位：加速度 m/s²，角速度 rad/s。
 */
...
void qmi8658_read_xyz(float acc[3], float gyro[3])
{
	...
	{
		data_ready = 1;
		qmi8658_delay_us(6);	// 等待 6 μs，确保同步采样数据稳定
	}
#else
	...
	else
	{
		...
		gyro[1] = g_imu.imu[4];
		gyro[2] = g_imu.imu[5];
		ESP_LOGD(tag, "QMI8658 sample not ready; retaining previous sample");
	}
}


/* 通过 CTRL7 启用或关闭加速度计、陀螺仪。 */
...
/* 读取并打印 CTRL1～CTRL8，便于核对初始化配置。 */
void qmi8658_dump_reg(void)
{
	unsigned char read_data[8] = {0};

	if (!qmi8658_read_reg(Qmi8658Register_Ctrl1, read_data, sizeof(read_data))) {
		ESP_LOGW(tag, "unable to read QMI8658 control registers");
		return;
	}
	ESP_LOGI(tag, "QMI8658 CTRL1..CTRL8: %02x %02x %02x %02x %02x %02x %02x %02x",
...
/* 扫描两个可能的 I2C 地址，校验芯片 ID，并读取版本及唯一标识。 */
unsigned char qmi8658_get_id(void)
{
	...
	while(iCount<2)
	{
		...
		{
			/* 地址探测失败属于正常情况，因此此处绕过带告警日志的通用读取接口。 */
			if(I2C_read_buff(g_imu.slave, Qmi8658Register_WhoAmI,
							&qmi8658_chip_id, 1) != ESP_OK)
			{
				qmi8658_chip_id = 0x00;
			}
		}
		...
		{
			ESP_LOGI(tag, "QMI8658 WhoAmI = 0x%02x", qmi8658_chip_id);
			...
			if (!qmi8658_read_reg(Qmi8658Register_Revision, &qmi8658_revision_id, 1)) {
				ESP_LOGW(tag, "unable to read QMI8658 revision");
			}
			qmi8658_write_reg(Qmi8658Register_Ctrl7, 0x00);
			qmi8658_write_reg(Qmi8658Register_Ctrl8, g_imu.cfg.ctrl8_value);
		ESP_LOGI(tag, "QMI8658 initialized at 0x%02x, revision 0x%02x",
...
/* 配置任意运动、无运动和显著运动检测所需的阈值及时间窗口。 */
void qmi8658_config_amd(void)
{
	g_imu.cfg.ctrl8_value &= (~QMI8658_CTRL8_ANYMOTION_EN);
	qmi8658_write_reg(Qmi8658Register_Ctrl8, g_imu.cfg.ctrl8_value);

	qmi8658_write_reg(Qmi8658Register_Cal1_L, 0x03);		// X 轴任意运动阈值
	qmi8658_write_reg(Qmi8658Register_Cal1_H, 0x03);		// Y 轴任意运动阈值
	qmi8658_write_reg(Qmi8658Register_Cal2_L, 0x03);		// Z 轴任意运动阈值
	qmi8658_write_reg(Qmi8658Register_Cal2_H, 0x02);		// X 轴无运动阈值
	...
	qmi8658_write_reg(Qmi8658Register_Cal4_H, 0x01);		// 写入第一组运动检测参数
	...
	qmi8658_write_reg(Qmi8658Register_Cal4_H, 0x02);		// 写入第二组运动检测参数

	qmi8658_send_ctl9cmd(qmi8658_Ctrl9_Cmd_Motion);
}

/* 启用任意运动检测，并将事件映射到指定中断引脚。 */
void qmi8658_enable_amd(unsigned char enable, enum qmi8658_Interrupt int_map, unsigned char low_power)
{
	...
	if(enable)
	{
		...
		if(int_map == qmi8658_Int1)
		{
			...
			qmi8658_write_reg(Qmi8658Register_Ctrl1, ctrl1);// 使能 INT1 输出
		}
		else if(int_map == qmi8658_Int2)
		{
			ctrl1 |= QMI8658_INT2_ENABLE;
			qmi8658_write_reg(Qmi8658Register_Ctrl1, ctrl1);// 使能 INT2 输出
		}
		g_imu.cfg.ctrl8_value |= QMI8658_CTRL8_ANYMOTION_EN;
		qmi8658_write_reg(Qmi8658Register_Ctrl8, g_imu.cfg.ctrl8_value);
...
void qmi8658_config_pedometer(unsigned short odr)
{
	...
	unsigned char ped_sig_count = 1; // 显著步数计数阈值
...
/* 将 FIFO 中完整的传感器帧读入 data，返回读取到的帧数。 */
...
unsigned char qmi8658_init(void)
{
	...
	{
		ESP_LOGE(tag, "QMI8658 device not found");
		return 0;
	}
}


/* QMI8658C 示例任务：每秒读取并打印一次六轴数据和温度。 */
